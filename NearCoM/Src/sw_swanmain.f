...
! start circulation module - fyshi
...
!        CALL PASS_SWAN2SHORECIRC(WaveHeightSW,WaveHeightGL)
          ...
          open(1,file='tmp.txt')
            DO IY=1,MYCGL
            write(1,1011)(WaveHeightGL(IX,IY),IX=1,MXCGL)
            ENDDO
          close(1)
          endif

1011      format(1000f12.3)
...
!
 ...
 6010 FORMAT (/,20X,'---------------------------------------',
     ...
     &        /,20X,'---------------------------------------',//)
!
      IF (SCREEN.NE.PRINTF.AND.INODE.EQ.MASTER) WRITE (SCREEN,6020)       40.30
...
!
!     *** setup flag ***                                                  32.02
! fyshi I set LSETUP =1 for calculation of radiation stresses
      LSETUP = 1                                                          32.02
!
...
!     ***  Default values for land point  ***
      ...
      IF (VARWLV) THEN
        COMPDA(1,JWLV2) = EXCFLD(7)                                       40.65 40.00
!       next two lines only for nonstat water level
        IF (JWLV1.GT.1) COMPDA(1,JWLV1) = EXCFLD(7)                       40.65 40.00
        IF (JWLV3.GT.1) COMPDA(1,JWLV3) = EXCFLD(7)                       40.65 40.00
      ENDIF
      IF (ICUR.GT.0) THEN
      ...
      ENDIF
      IF (VARFR) THEN
        COMPDA(1,JFRC2) = EXCFLD(4)                                       40.65 40.00
        COMPDA(1,JFRC3) = EXCFLD(4)                                       40.65 40.00
      ENDIF
      IF (VARWI) THEN
      ...
      ENDIF
      COMPDA(1,JBOTLV) = EXCFLD(1)                                        40.65
...
!       --- "GE" changed to "EQ" since OPTG.EQ.3 FOR 1          40.08
        IF (OPTG.EQ.3)                                                    40.08
...
!   remove reading water level when master coupling - fyshi
!     field 7: water level
      IFLDYN(7)=1
...
!       Add bottom level to obtain depth
 ...
 41       CONTINUE
 31     CONTINUE
!  test if water level pass to swan
!       write(*,*) COMPDA(KGRPNT(10,10),JWLV2)
!     &         ,COMPDA(KGRPNT(10,10),JDP2)
...
! remove read u and v from file - fyshi
