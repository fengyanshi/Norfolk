...
!      - CONSTRUCT_X
!      - CONSTRUCT_Y
!    Last Update: 12/12/2013 Fengyan Shi, University of Delaware
! --------------------------------------------------
SUBROUTINE CONSTRUCTION
     USE GLOBAL
     IMPLICIT NONE
     REAL(SP),DIMENSION(Mloc1,Nloc) :: BEDxL,BEDxR
     REAL(SP),DIMENSION(Mloc,Nloc1) :: BEDyL,BEDyR
...
! in case water falls, pressure gradient is calculated using gradient of eta and bed 

     DO I=1,Mloc1
     DO J=1,Nloc
      BEDxL(I,J)=MAX(EtaRxL(I,J),-Depthx(I,J))
      BEDxR(I,J)=MAX(EtaRxR(I,J),-Depthx(I,J))
     ENDDO
     ENDDO

     DO I=1,Mloc
     DO J=1,Nloc1
      BEDyL(I,J)=MAX(EtaRyL(I,J),-Depthy(I,J))
      BEDyR(I,J)=MAX(EtaRyR(I,J),-Depthy(I,J))
     ENDDO
     ENDDO
     ...
     FxL=PL*UxL+0.5_SP*GRAV*((BEDxL)*(BEDxL)+2.0_SP*(BEDxL)*(Depthx))*JL11x
     FxR=PR*UxR+0.5_SP*GRAV*((BEDxR)*(BEDxR)+2.0_SP*(BEDxR)*(Depthx))*JL11x
     GxL=PL*VxL+0.5_SP*GRAV*((BEDxL)*(BEDxL)+2.0_SP*(BEDxL)*(Depthx))*JL12x
     GxR=PR*VxR+0.5_SP*GRAV*((BEDxR)*(BEDxR)+2.0_SP*(BEDxR)*(Depthx))*JL12x
! ANNA start
...
! I changed to JS2 ???
     ...
     FyL=QL*UyL+0.5*GRAV*((BEDyL)*(EtaRyL)+2.0_SP*(BEDyL)*(Depthy))*JL21y
     FyR=QR*UyR+0.5*GRAV*((BEDyR)*(EtaRyR)+2.0_SP*(BEDyR)*(Depthy))*JL21y
     GyL=QL*VyL+0.5*GRAV*((BEDyL)*(EtaRyL)+2.0_SP*(BEDyL)*(Depthy))*JL22y
     GyR=QR*VyR+0.5*GRAV*((BEDyR)*(EtaRyR)+2.0_SP*(BEDyR)*(Depthy))*JL22y
! ANNA start
