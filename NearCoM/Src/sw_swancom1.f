...
!
 ...
 8510   FORMAT(' The ',A9,                                                40.41
     &         ' convention for wind and wave directions is used')        40.41
        IF (PROPSC.EQ.3) THEN                                             40.41
          WRITE(PRINTF,8511) 'S&L   '                                     40.41
          ICMAX  = 10                                                     40.41
        ELSE IF (PROPSC.EQ.2) THEN                                        40.41
...
!
            IF (SCREEN.NE.PRINTF) THEN
              IF (NSTATC.EQ.1) THEN                                       40.00
                IF (INODE.EQ.MASTER) WRITE(SCREEN,313) CHTIME, IT,        40.30
     &                                                 ITER, SWPDIR       40.30
...
!MPI!                                                                         40.30
           ...
           IF (NSTATC.EQ.0 .AND. NPFL.GT.0 .AND. INODE.EQ.MASTER)         40.23
...
!                                                                         40.23
           ...
           IF (NSTATC.EQ.0 .AND. NPFR.GT.0 .AND. INODE.EQ.MASTER)         40.23
...
!
        IF (PNUMS(21).EQ.1. .AND. ITER.EQ.1) THEN                         40.41
           ...
           IF (NSTATC.EQ.0.AND.INODE.EQ.MASTER) WRITE(SCREEN,113)         40.41
        ELSE                                                              40.41
           WRITE(PRINTF,112) ACCUR,PNUMS(4)                               30.72
           IF (NSTATC.EQ.0.AND.INODE.EQ.MASTER)                           40.30
...
!
      IF (.NOT.CSETUP) THEN                                               30.82
          ...
          IF (SCREEN.NE.PRINTF.AND.NSTATC.EQ.0.AND.INODE.EQ.MASTER)       40.30 40.13
...
!
      USE OCPCOMM4                                                        40.41
      USE SWCOMM3   
      USE PASS, ONLY : WaveFxSW,WaveFySW,WaveAngleSW,PeakPeriodSW,
     &                 PeakAngleSW,WaveFluxXSW,WaveFluxYSW,AvePeriodSW                                
!
      IMPLICIT NONE
!
!
...
!
      ...
      REAL     TMP1,TMP2,TMP3,TMP4,TMPmax,TMPmax1,TMPpeakangle,
     &         TMPfluxX,TMPfluxY,TMPper,TMPeng
...
! calculate mean angle  - fyshi
! --------------------------------------------
!   according to sw_swanser.fsw
...
!    spcsig is 2*pi*f
          ...
          TMP1=TMP1+SPCSIG(IS)**2*AC2(ID,IS,INDX)
     &               *FRINTF*DDIR*SPCDIR(ID,2)
          TMP2=TMP2+SPCSIG(IS)**2*AC2(ID,IS,INDX)
     &               *FRINTF*DDIR*SPCDIR(ID,3)
!         energy
          TMP3=SPCSIG(IS)**2*AC2(ID,IS,INDX)
     &              *FRINTF*DDIR
! for mean period
          TMPeng=TMPeng+SPCSIG(IS)**2*AC2(ID,IS,INDX)
     &              *FRINTF*DDIR
          TMPper=TMPper+SPCSIG(IS)**2*AC2(ID,IS,INDX)
     &         *FRINTF*DDIR*SPCSIG(IS)/2.0/3.1415926

!         flux  note Hrms^2=8.0*energy
!   flux = B0*gH^2/Cg  where B0=0.125
          IF(CG(1).EQ.0.)THEN
            WRITE(*,*)'CG=0, STOP'
          ENDIF
          TMPfluxX=TMPfluxX+0.125*9.8*8.0*TMP3/CG(1)*SPCDIR(ID,2)
          TMPfluxY=TMPfluxY+0.125*9.8*8.0*TMP3/CG(1)*SPCDIR(ID,3)

!         max inside a frequency component
          IF(TMP3.GT.TMPmax1)THEN
            TMPmax1=TMP3
            TMPpeakangle=SPCDIR(ID,1)*180.0/PI
          ENDIF

          TMP4=TMP3+TMP4
         ENDDO  ! end ID

             IF(TMP4.GT.TMPmax)THEN
              TMPmax=TMP4
              PeakAngleSW(IX,IY) = TMPpeakangle
              PeakPeriodSW(IX,IY) = 2.0*PI/SPCSIG(IS)
             ENDIF

         ENDDO  ! end IS

           WaveAngleSW(IX,IY) = ATAN2(TMP2,TMP1)*180.0/PI
           WaveFluxXSW(IX,IY) = TMPfluxX
           WaveFluxYSW(IX,IY) = TMPfluxY   
           AvePeriodSW(IX,IY) = TMPeng/MAX(1.0E-12,TMPper)
...
!         do IY=1,MYC
!           write(10,121)(AvePeriodSW(IX,IY),IX=1,MXC)
!         ENDDO
!        close(10)
!121     format(200f12.3)
...
!
! fyshi, pass wave forcing to WaveFxSW, WaveFySW
! note that this part of swan is different from the previous version, 3 and 4 used in previous
                 WaveFxSW(IX,IY)=SETPDA(INDX,1)
                 WaveFySW(IX,IY)=SETPDA(INDX,2)
              END IF
           END DO
        END DO
